...
class LogIngestionService:
    def __init__(self, internal_batch_size: int = 10):
    ...
    def ingest_log(self, log_object: Logs) -> Logs:
        ...
        # push to redis after checking batch size threshold
        if current_batch_size >= self.internal_batch_size:
            logging.info(f"Batch size reached threshold | flushing_to_redis | batch_size={current_batch_size}")
            self.flush_cache_to_redis()

        return log_object

    def flush_cache_to_redis(self) -> None:
        batch_size = len(self.batch_caching.cache)
        start = time.time()
        ...
        logging.info(f"Redis flush completed | flushed_count={batch_size} | duration={duration}s")
        return True
    ...
    def flush_redis_to_clickhouse(self) -> bool:
        ...
        if success_count == redis_count:
            ...
            logging.warning(
                "Partial ClickHouse insert detected | Redis NOT cleared to prevent data loss"
            )
            return False
